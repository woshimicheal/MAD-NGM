...
class Problem:
    def __init__(self, probName, sampleName, N, nrLayers):
        self.N = N
    ...
    def setupBurgers(self):
        '''Burgers with exact solution'''
        self.OmegaPeriodic = 1
        self.dim = 2
        self.Omega =  jnp.array([0., 1.]).reshape((2, 1))
        self.OmegaInit = self.Omega
        self.maxDiffDegree = 2
        self.Tend = 1.0
        self.u0 = lambda x: -jnp.sin(jnp.pi*x)
        self.uT = lambda x,t: -jnp.sin(jnp.pi*x)
        self.plotGrid = jax.numpy.linspace(self.Omega[0, 0], self.Omega[1, 0], num = 1024).reshape((-1,1));
        self.plotGridInit = jax.numpy.linspace(self.OmegaInit[0, 0], self.OmegaInit[1, 0], num = 10240).reshape((-1,1));
    ...
    def setupHeat(self):
        self.OmegaPeriodic = 1
        self.dim = 2
        self.Omega = jnp.array([self.dim *[0.0],self.dim*[1.0]])
        self.Tend = 1
        self.v = 1
        self.getInitRBFp = []
        self.getInitRBF = []
    ...
    def setupParticleTrapTime(self, d):
        ...
        self.nu = lambda t: 1.25*(jnp.sin(jnp.pi*t) + 1.5)
        self.alpha = 0.25
        self.beta = 100.
...
def sampleGaussHelper(Omega, Nx, key, phi, alpha, Z, knots):
    N = alpha.size
    NStep = int(jnp.ceil(Nx/N))
    xSamples = jnp.zeros((NStep*N,))
    bw = (Omega[1] - Omega[0])/2.
    ...
    for i in range(N):
        ...
        sigmaU = 2*jnp.sqrt(jnp.mean((y - meanU)**2*normalizingConstant))
        key, subkey = jax.random.split(key)

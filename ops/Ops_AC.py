...
class OpsAc:
    ...
    def rhsJF1(self, x, alphaZ, t):
        return AcRHS(self.dnn.ufunScalar, self.dnn.ufun, self.bc, self.prob.v, self.prob.nu,x.reshape((-1,)), alphaZ, t)
    
    def rhsJF2(self, x, alphaZ, Latent,delta, t,key):
        return AcRHS_Latent(self.dnn.ufunLatentScalar, self.dnn.ufunLatent, self.bc,self.prob.nu, self.prob.v,  x.reshape((-1,)), alphaZ,Latent,delta, t,key)

    def rhsJF3(self, x, Latent_alphaZ, delta,t,key):
        return AcRHS(self.dnn.ufunWithLatentScalar, self.dnn.ufunWithLatent, self.bc, self.prob.v,self.prob.nu, x.reshape((-1,)), Latent_alphaZ,delta, t,key)


@partial(jit, static_argnums=(0,1,2,3))
def AcRHS_Latent(ufunLantentScalar,ufunLatent,bc,nu,v,x,alphaZ,Latent,delta,t,key):
    ...
    f = nu*(u**3- u) - v * dxx
    # f = nu(x,t,delta)*(u**3- u) - v * dxx
    J = jnp.linalg.lstsq(Jac,-f,rcond=1e-4)[0]

...
class Net(torch.nn.Module):
    ...
    def forward(self, x ,delta=0,selected_latent=None):
...
class MadNN():
    def __init__(self, u0Mat,xx0,delta,layers,latent_size=0,num_latents=1,maxiter=1000,model=None,para_dict=None,Latent =None):
        self.xx0 = torch.tensor(xx0,requires_grad=True).float().to(device)
        self.delta = torch.tensor(delta).float().to(device)
    ...
    def Latents_pretrain(self):
        ...
        # self.net.register_parameter('Latents', latents)  
        net.register_parameter('Latents', latents) 
        self.net = net.to(device)

    def net_u(self,index=None):
        selected_latent = self.net.Latents[index]
        X = self.xx0[index].unsqueeze(-1).float().to(device) 
        delta = self.delta[index] 
        u = self.net.forward(X,delta,selected_latent).to(device)  
        return u
...
def data_loader1(file_load,s=128):
...
def Mad_LoadData(file_load,s =200 ):
    ...
    sub = 4
    xx_test = xx[:,::sub]
    Input_test = Input[:,::sub]    
    Outpu_test = Outpu[:,::2,::sub]  
    u0Mat = Input_test

    return u0Mat,xx_test,de

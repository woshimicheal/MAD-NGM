...
def data_loader(file_load,s=128):
    ...
    tt_test = tt_test.flatten()[:,None]
    Input = Input[:,::sub]
    Outpu = Outpu[:,:,::sub]
    xx_test = jnp.array(xx_test)  
    tt_test = jnp.array(tt_test)
    Input_test = jnp.array(Input)  
    Outpu_test = jnp.array(Outpu)  
    return xx_test,Input_test,tt_test,Outpu_test
